...
import { impactScores, readingStages, type ReadingStage } from "@/lib/constants";
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <Card className="space-y-6 xl:col-span-2 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(247,244,239,0.9))]">
        ...
        <div className="h-[380px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={analytics.stackedByDomainAndStage} margin={{ top: 16, right: 16, left: 0, bottom: 8 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#d7e0e5" />
              ...
              {readingStages.map((stage) => (
                <Bar key={stage} dataKey={stage} stackId="a" fill={stageColors[stage]} radius={[8, 8, 0, 0]} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        </div>
