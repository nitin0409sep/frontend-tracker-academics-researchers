import { datePresets, impactScores, readingStages, researchDomains, type DatePreset } from "@/lib/constants";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Filter Library</p>
          <h3 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-foreground">Choose filters</h3>
      ...
      <div className="grid gap-4">
        <FilterRow
          ...
          onToggle={(value) => onChange({ ...filters, readingStage: toggleValue(filters.readingStage, value) })}
        />
        <FilterRow
          label="Research Domain"
          items={researchDomains}
          activeItems={filters.researchDomain}
          onToggle={(value) => onChange({ ...filters, researchDomain: toggleValue(filters.researchDomain, value) })}
        />
        <FilterRow
          label="Impact Score"
          items={impactScores}
          activeItems={filters.impactScore}
          onToggle={(value) => onChange({ ...filters, impactScore: toggleValue(filters.impactScore, value) })}
        />
