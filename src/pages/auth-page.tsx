...
export function AuthPage() {
  ...
  async function handleSignup(values: SignupValues) {
    try {
      await signup(values);
      pushToast({ variant: "success", title: "Account created", message: "Your secure workspace is ready." });
    } catch (error) {
      pushToast({ variant: "error", title: "Sign up failed", message: error instanceof Error ? error.message : "Unable to create account." });
    }
  }
  ...
  async function handleLogin(values: LoginValues) {
    try {
      await login(values);
      pushToast({ variant: "success", title: "Signed in", message: "Welcome back to your paper tracker." });
    } catch (error) {
      pushToast({ variant: "error", title: "Sign in failed", message: error instanceof Error ? error.message : "Unable to sign in." });
    }
  }
        ...
        <Card className="overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(182,219,212,0.85),transparent_30%),radial-gradient(circle_at_bottom_right,rgba(234,214,180,0.55),transparent_24%),linear-gradient(135deg,#f7f5ef_0%,#eef5f3_100%)] p-8">
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              ...
              <h1 className="text-4xl font-semibold tracking-[-0.05em] text-foreground">Protected research workspace</h1>
              <p className="max-w-xl text-sm leading-7 text-muted-foreground">
                Sign in to access your personal paper library, analytics, and protected create-paper workflow.
              </p>
            </div>
          </div>
        </Card>
          ...
          {isSignup ? (
            <form className="space-y-4" onSubmit={signupForm.handleSubmit(handleSignup)} noValidate>
              <Field label="Full Name" error={signupForm.formState.errors.fullName?.message}>
                <Input {...signupForm.register("fullName")} className={fieldClassName(Boolean(signupForm.formState.errors.fullName))} />
              </Field>
              <Field label="Email" error={signupForm.formState.errors.email?.message}>
                <Input type="email" {...signupForm.register("email")} className={fieldClassName(Boolean(signupForm.formState.errors.email))} />
              </Field>
              <Field label="Password" error={signupForm.formState.errors.password?.message}>
                <Input type="password" {...signupForm.register("password")} className={fieldClassName(Boolean(signupForm.formState.errors.password))} />
              </Field>
            ...
            <form className="space-y-4" onSubmit={loginForm.handleSubmit(handleLogin)} noValidate>
              <Field label="Email" error={loginForm.formState.errors.email?.message}>
                <Input type="email" {...loginForm.register("email")} className={fieldClassName(Boolean(loginForm.formState.errors.email))} />
              </Field>
              <Field label="Password" error={loginForm.formState.errors.password?.message}>
                <Input type="password" {...loginForm.register("password")} className={fieldClassName(Boolean(loginForm.formState.errors.password))} />
              </Field>
