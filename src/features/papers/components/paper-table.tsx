...
import type { DatePreset, Paper } from "@/shared/lib/types"
    ...
    <Card className="flex min-h-0 flex-col overflow-hidden p-0 lg:h-full">
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-end gap-4">
              ...
              <p className="pb-1 text-sm text-muted-foreground">
                paper{papers.length === 1 ? "" : "s"} in view
              </p>
            </div>
          </div>
        ...
        <table className="w-full table-fixed border-collapse">
          ...
          <tbody>
            {sortedPapers.map((paper) => (
              <tr
                key={paper.id}
                className="border-b border-border/90 align-top transition hover:bg-[#fafbf8] last:border-b-0"
              >
      ...
      <div className="app-scrollbar grid flex-1 gap-4 overflow-y-auto p-4 sm:grid-cols-2 xl:hidden">
        {sortedPapers.map((paper) => (
          <Card
            key={paper.id}
            className="space-y-5 border border-white/90 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(244,247,245,0.94))] p-5 shadow-soft"
          >
            <div className="space-y-3">
